...
try:
    from ..paths import (
        DICTIONARIES_DIR,
        DICTIONARY_DOWNLOADS_DIR,
        DICTIONARY_MANIFEST_PATH,
        DICTIONARY_SOURCES_EXAMPLE_PATH,
...
def installed_dictionaries_dir(extension_root: str | Path) -> Path:
    return INSTALLED_DICTIONARIES_DIR


def dictionary_manifest_path(extension_root: str | Path) -> Path:
    return DICTIONARY_MANIFEST_PATH
...
def download_dictionary_url(
    ...
    *,
    ...
    tmp_root = DICTIONARY_DOWNLOADS_DIR
...
def write_source_template(extension_root: str | Path) -> Path:
    path = DICTIONARY_SOURCES_EXAMPLE_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "notes": [
