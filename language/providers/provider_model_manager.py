...
try:
    from ..constants import (
        ...
        NLLB_MODEL_IDS,
        OFFICIAL_OFFLINE_PROVIDERS,
        PROVIDER_ID_ARGOS,
        PROVIDER_ID_NLLB,
        QUALITY_COMPARE_PROVIDERS,
    )
    from ..paths import ARGOS_MODELS_DIR, MODELS_DIR
except ImportError:
    try:
        ...
        from language.paths import ARGOS_MODELS_DIR, MODELS_DIR
    except ImportError:
        from constants import (
            DEFAULT_NLLB_MODEL_ID,
            DEFAULT_PROVIDER_CHAIN,
...
def install_nllb_model(*, models_dir: str | Path = MODELS_DIR, model_id: str = DEFAULT_NLLB_MODEL_ID, force: bool = False, revision: Optional[str] = None) -> bool:
...
def get_nllb_matrix(pairs: Optional[Iterable[Tuple[str, str]]] = None, *, models_dir: str | Path = MODELS_DIR, model_id: str = DEFAULT_NLLB_MODEL_ID) -> List[PairInfo]:
    requested = normalize_pairs(pairs or get_recommended_pairs("lightweight"))
    local = nllb_local_dir(models_dir, model_id)
...
def install_nllb_bundle(pairs: Iterable[Tuple[str, str]], *, models_dir: str | Path = MODELS_DIR, model_id: str = DEFAULT_NLLB_MODEL_ID, force: bool = False) -> InstallResult:
    requested = normalize_pairs(pairs)
    local = nllb_local_dir(models_dir, model_id)
...
def write_provider_config_template(path: Path) -> Dict[str, Any]:
    config = {
        ...
        "official_offline_providers": OFFICIAL_OFFLINE_PROVIDERS,
        ...
        "argos": {"python_package": "argostranslate", "models_dir": str(ARGOS_MODELS_DIR), "auto_install_dependency": False, "auto_download_models": False},
        "nllb": {"python_packages": ["transformers", "sentencepiece", "huggingface_hub"], "models_dir": str(MODELS_DIR), "default_model": DEFAULT_NLLB_MODEL_ID, "auto_install_dependency": False, "auto_download_models": False, "install_torch": False},
        "quality": {"enabled": True, "mode": "prompt", "compare_providers": QUALITY_COMPARE_PROVIDERS},
    }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p_nllb_matrix.add_argument("--models-dir", default=str(MODELS_DIR))
    p_nllb_matrix.add_argument("--model", default=DEFAULT_NLLB_MODEL_ID)

    p_install_nllb_model = sub.add_parser("install-nllb-model")
    p_install_nllb_model.add_argument("--models-dir", default=str(MODELS_DIR))
    ...
    p_install_nllb_bundle.add_argument("--models-dir", default=str(MODELS_DIR))
